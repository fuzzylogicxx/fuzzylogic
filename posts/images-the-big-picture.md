...
1. Check the Zach post which has good additions to the above for when you combine HTML with and height attrs, srcset and max-width: 100%. It seems that width: 100% trumps the HTML width. But max-width:100% will not go 100% even if there are images in sizes to allow it to IF the width attribute is a value which is less than than 100% container width. You can get around this by picking a large/r width and height or by including width: auto. 
